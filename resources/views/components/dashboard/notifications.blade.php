    ...
    >
        <div class="px-4 py-2 border-t border-gray-200">
            ...
            @forelse($notifications as $notif)
                <a href="{{ $notif->url ?? '#' }}" class="block px-2 py-1 text-sm text-gray-600 hover:bg-gray-100 rounded">
                    {{ $notif->content }}
                    @if(!$notif->read)
            ...
            <div class="mt-2 text-end">
                <a href="{{ $link }}" class="text-xs text-blue-600 hover:underline">Voir toutes les notifications</a>
            </div>
        </div>
    </div>
</div>